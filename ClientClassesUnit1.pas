...
type
  ...
  private
    ...
    FVerifyCodeCommand: TDBXCommand;
  ...
  public
    ...
    function VerifyCode(NameDataSet: string; CodeName: string; CodeValue: string; KeyName: string; KeyValue: Integer): Boolean;
    function GetAvailableDatabases(CualEstado: Byte): string;
    function GetLastId: Integer;
    function GetLastCode: string;
    function GetId(Table: string; FieldName: string): Integer;
...
function TServerMethods1Client.VerifyCode(NameDataSet: string; CodeName: string; CodeValue: string; KeyName: string; KeyValue: Integer): Boolean;
begin
  if FVerifyCodeCommand = nil then
  begin
    FVerifyCodeCommand := FDBXConnection.CreateCommand;
    FVerifyCodeCommand.CommandType := TDBXCommandTypes.DSServerMethod;
    FVerifyCodeCommand.Text := 'TServerMethods1.VerifyCode';
    FVerifyCodeCommand.Prepare;
  end;
  FVerifyCodeCommand.Parameters[0].Value.SetWideString(NameDataSet);
  FVerifyCodeCommand.Parameters[1].Value.SetWideString(CodeName);
  FVerifyCodeCommand.Parameters[2].Value.SetWideString(CodeValue);
  FVerifyCodeCommand.Parameters[3].Value.SetWideString(KeyName);
  FVerifyCodeCommand.Parameters[4].Value.SetInt32(KeyValue);
  FVerifyCodeCommand.ExecuteUpdate;
  Result := FVerifyCodeCommand.Parameters[5].Value.GetBoolean;
end;
...
function TServerMethods1Client.GetId(Table: string; FieldName: string): Integer;
begin
  ...
  begin
    ...
    FGetIdCommand.Text := 'TServerMethods1.GetId';
    FGetIdCommand.Prepare;
  end;
  FGetIdCommand.Parameters[0].Value.SetWideString(Table);
  FGetIdCommand.Parameters[1].Value.SetWideString(FieldName);
  FGetIdCommand.ExecuteUpdate;
  Result := FGetIdCommand.Parameters[2].Value.GetInt32;
end;
