...
Type
  TClasePermisos = (vpLeer, vpEscribir, vpInsertar, vpBorrar, vpImprimir, vpExportar, vpImportar, vpTodos, vpNinguno);
  ...
  TErrorAccesoDatos = (EADMessage, EADError, EADNone);
  ...
  private
    ...
    function FuncVerificaCodigo(DataSet: TClientDataSet; CodigoNombre: String;
      CodigoValor: String; LlaveNombre: String; LlaveValor: Integer): Boolean;
    ...
    function FuncDevuelveId(Tabla: String; CampoBuscar: String): LongInt;
...
function VerificaCodigo(DataSet: TClientDataSet; CodigoNombre: String;
  CodigoValor: String; LlaveNombre: String; LlaveValor: Integer): Boolean;
...
function DevuelveId(Tabla: String; Campo: String): LongInt;
...
uses UFrmParamsConfig, Unit2, TypInfo, UTFrmMenuDX;

Const
  DefaultMaxCuantos = 100;

function Valida_Bit(const Valor: Cardinal; const Bit: Byte): Boolean;
begin
  Result := (Valor and (1 shl Bit)) <> 0;
end;
...
procedure TClientModule1.VerificaDataSetEmbarcacion;
begin
  ...
  if Not cdEmbarcacion.Active then
    cdEmbarcacion.Open;
end;

procedure TClientModule1.VerificaDataSetUnidades;
begin
...
function TClientModule1.FuncVerificaCodigo(DataSet: TClientDataSet; CodigoNombre: String;
  CodigoValor: String; LlaveNombre: String; LlaveValor: Integer): Boolean;
var
  Proxy: TServerMethods1Client;
begin
  ...
  Try
    Proxy := TServerMethods1Client.Create(ClientModule1.SQLConnection1.DBXConnection);
    Result := proxy.VerifyCode(DataSet.ProviderName, CodigoNombre, CodigoValor, LlaveNombre, LlaveValor);
  Finally
    proxy.Free;
  End;
end;
...
function TClientModule1.FuncDevuelveId(Tabla: String; CampoBuscar: String): LongInt;
var
  Proxy: TServerMethods1Client;
begin
  Result := -1;
  Try
    Proxy := TServerMethods1Client.Create(ClientModule1.SQLConnection1.DBXConnection);
    Result := Proxy.GetId(Tabla, CampoBuscar);
  Finally
    Proxy.Free;
  end;
end;
...
begin
  EliminarConjunto([cdEmbarcacion, cdUnidades]);
  MisTablas.Destroy;
  ListaLapso.Free;
end;
...
procedure TClientModule1.ImprimeReporte(NombreReporte: String; Reporte: TfrxReport; var pRuta: String);
begin
  ImprimeReporte(NombreReporte, Reporte, pRuta, Nil);
end;

procedure TClientModule1.ImprimeReporte(NombreReporte: String; Reporte: TfrxReport; var pRuta: String; Preview: TfrxPreview);
var
  Ruta, Cadena: String;
  Archivo: TSearchRec;
  Resultado: Integer;
...
function VerificaCodigo(DataSet: TClientDataSet; CodigoNombre: String;
  CodigoValor: String; LlaveNombre: String; LlaveValor: Integer): Boolean;
begin
  Result := ClientModule1.FuncVerificaCodigo(DataSet, CodigoNombre, CodigoValor, LlaveNombre, LlaveValor);
end;
...
function DevuelveId(Tabla: String; Campo: String): LongInt;
begin
  Result := ClientModule1.FuncDevuelveId(Tabla, Campo);
end;
...
function DescomponerFecha(Fecha: TDate = 0; FechaHora: TDateTime = 0; DiaSemana: Boolean = False; Modo: TModoFecha = tmMinuscula): String;
const
  NombreDia: Array[1..7] of String = ('Domingo', 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado');
...
procedure VerificaDataSetEmbarcacion;
begin
  ClientModule1.VerificaDataSetEmbarcacion;
end;

procedure VerificaDataSetUnidades;
begin
  ClientModule1.VerificaDataSetUnidades;
end;
...
var
  Resultado: Boolean;
  i: Integer;
  NoCuenta: TStringList;

function Leyenda(Cual: TClasePermisos): String;
begin
  case Cual of
    vpLeer: Result := 'LECTURA - Permite la visualización de la información.';
    vpEscribir: Result := 'EDICIÓN - Permite la modificación de la información.';
    vpInsertar: Result := 'INSERCIÓN - Permite agregar registros de información.';
    vpBorrar: Result := 'BORRADO - Permite la eliminación de información.';
    vpImprimir: Result := 'IMPRESIÓN - Permite enviar a reporte impreso la información.';
    vpExportar: Result := 'EXPORTAR - Permite enviar información hacia un archivo externo.';
    vpImportar: Result := 'IMPORTAR - Permite recibir información desde un archivo externo.';
  end;
end;

begin
  if ClientModule1.cdPermisos.Locate('TABLE_NAME', Tabla, []) then
  begin
    i := low(Permiso);
    Resultado := True;
    NoCuenta := TStringList.Create;
    NoCuenta.Clear;
    while (i <= high(Permiso)) do
    begin
      if Not Valida_Bit(ClientModule1.cdPermisos.FieldByName('Permisos').AsInteger, Byte(Permiso[i])) then
      begin
        NoCuenta.Add(Leyenda(permiso[i]));
        Resultado := False;
      end;
    ...
    try
      ...
      begin
        if TipoMensaje = EADMessage then
          InteliDialog.ShowModal('Usuario no cuenta con permisos suficientes', 'El Usuario no cuenta con permisos de: ' + #10 + #10 + StringReplace(NoCuenta.CommaText, ',', #10, [rfReplaceAll, rfIgnoreCase]) + #10 + #10 + 'El administrador del sistema los ha establecido como requisitos mínimos para acceder/manipular los datos de:' + #10 + ClientModule1.cdPermisos.FieldByName('Descripcion').AsString, mtInformation, [mbOk], 0);

        if TipoMensaje = EADError then
          raise InteligentException.CreateByCode(33, [StringReplace(NoCuenta.CommaText, ',', #10, [rfReplaceAll, rfIgnoreCase]), ClientModule1.cdPermisos.FieldByName('Descripcion').AsString]);
      end;
    Finally
      Result := Resultado;
    end;
  ...
  begin
    Result := False;
    if TipoMensaje = EADMessage then
      InteliDialog.ShowModal('Usuario no cuenta con permisos suficientes', 'No se ha especificado ningún permiso para el usuario en este elemento.', mtInformation, [mbOk], 0);
