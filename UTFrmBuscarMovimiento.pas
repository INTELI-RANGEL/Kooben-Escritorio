...
type
  TFrmBuscarMovimiento = class(TForm)
...
var
  FrmBuscarMovimiento: TFrmBuscarMovimiento;
...
procedure TFrmBuscarMovimiento.btnAbrirClick(Sender: TObject);
begin
  Resultado := mrOk;
  Close;
end;

procedure TFrmBuscarMovimiento.btnArribaClick(Sender: TObject);
begin
  btnActual := btnArriba;    // Establecer por default la busqueda hacia abajo
  ProseguirBusqueda;
end;

procedure TFrmBuscarMovimiento.btnCancelarClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmBuscarMovimiento.btnAbajoClick(Sender: TObject);
begin
  btnActual := btnAbajo;    // Establecer por default la busqueda hacia abajo
  ProseguirBusqueda;
end;

procedure TFrmBuscarMovimiento.cbVistaChange(Sender: TObject);
begin
...
procedure TFrmBuscarMovimiento.eBuscarKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  Ind: Integer;
begin
...
procedure TFrmBuscarMovimiento.FormClose(Sender: TObject; var Action: TCloseAction);
var
  i: Integer;
begin
  ...
  begin
    Recientes.Insert(0, dsMovimiento.DataSet.FieldByName('IdRegistroMovimiento').AsString);
...
procedure TFrmBuscarMovimiento.FormCreate(Sender: TObject);
begin
  Resultado := mrCancel;
end;

procedure TFrmBuscarMovimiento.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  // Tecla <ESC>
...
procedure TFrmBuscarMovimiento.FormShow(Sender: TObject);
var
  i: Integer;
begin
  Try
...
function TFrmBuscarMovimiento.funcPregunta(Nodo: TAdvGlowButton): String;
begin
...
procedure TFrmBuscarMovimiento.IconosClick(Sender: TObject);
begin
...
procedure TFrmBuscarMovimiento.IconosPequenosClick(Sender: TObject);
begin
  lvRecetas.ViewStyle := vsSmallIcon;
  lvRecientes.ViewStyle := vsSmallIcon;
...
procedure TFrmBuscarMovimiento.ListaClick(Sender: TObject);
begin
...
function TFrmBuscarMovimiento.LocalizaNodo(Key: Word; Shift: TShiftState;
...
procedure TFrmBuscarMovimiento.lvRecetasChange(Sender: TObject; Item: TListItem;
  Change: TItemChange);
begin
  if sender is TListView then
    if Assigned(TListView(Sender).Selected) then
      dsMovimiento.DataSet.Locate('IdRegistroMovimiento', Integer(TListView(Sender).Selected.Data), []);
...
procedure TFrmBuscarMovimiento.lvRecetasContextPopup(Sender: TObject;
...
procedure TFrmBuscarMovimiento.lvRecetasDblClick(Sender: TObject);
begin
  if btnAbrir.Enabled then
    btnAbrir.Click;
end;

procedure TFrmBuscarMovimiento.lvRecientesContextPopup(Sender: TObject;
  MousePos: TPoint; var Handled: Boolean);
begin
  GetCursorPos(MousePos);
  popVisualizacion.Popup(MousePos.X, MousePos.Y);
...
procedure TFrmBuscarMovimiento.PcRecetasChange(Sender: TObject);
begin
...
procedure TFrmBuscarMovimiento.ProseguirBusqueda;
var
  Ind: Integer;
begin
...
procedure TFrmBuscarMovimiento.ReporteClick(Sender: TObject);
begin
...
procedure TFrmBuscarMovimiento.tsRecientesShow(Sender: TObject);
begin
  btnAbrir.Enabled := (lvRecetas.CanFocus and (lvRecetas.Items.Count > 0)) or (lvRecientes.CanFocus and (lvRecientes.Items.Count > 0));
end;

procedure TFrmBuscarMovimiento.tsTodasShow(Sender: TObject);
begin
  if Not YaLeidas then
  begin
    try
      dsMovimiento.DataSet.DisableControls;
      ...
      dsMovimiento.DataSet.EnableControls;
    end;
  end;
end;

procedure TFrmBuscarMovimiento.CargarListaRecetas(ListView: TListView);
var
  Nodo: TListItem;
  Lista: TStringList;
  Grupo: TListGroup;
...
begin
  ...
  try
    ...
    dsMovimiento.DataSet.First;
    while not dsMovimiento.DataSet.Eof do
    begin
      if (CompareText(ListView.Name, 'lvRecetas') = 0) or (Recientes.IndexOf(dsMovimiento.DataSet.FieldByName('IdRegistroMovimiento').AsString) >= 0) then
      begin
        IdGpo := Lista.IndexOf(dsMovimiento.DataSet.FieldByName('CodigoComisaria').AsString);
        if IdGpo < 0 then
        begin
          Grupo := ListView.Groups.Add;
          Grupo.Header := dsMovimiento.DataSet.FieldByName('CodigoComisaria').AsString;
          Grupo.Subtitle := dsMovimiento.DataSet.FieldByName('Referencia').AsString;
          Grupo.TitleImage := 1;

          IdGpo := Grupo.Index;

          Lista.AddObject(dsMovimiento.DataSet.FieldByName('CodigoComisaria').AsString, TObject(IdGpo));
        end
        else
          IdGpo := Integer(Lista.Objects[IdGpo]);

        Nodo := ListView.Items.Add;
        Nodo.Caption := dsMovimiento.DataSet.FieldByName('CodigoRegistroMovimiento').AsString;
        Nodo.ImageIndex := 11;
        Nodo.SubItems.Add(dsMovimiento.DataSet.FieldByName('Referencia').AsString);
        Nodo.SubItems.Add(dsMovimiento.DataSet.FieldByName('Aplicacion').AsString);
        Nodo.SubItems.Add(dsMovimiento.DataSet.FieldByName('Comentarios').AsString);
        Nodo.Data := TObject(dsMovimiento.DataSet.FieldByName('IdRegistroMovimiento').AsInteger);
        Nodo.GroupID := IdGpo;
      end;

      dsMovimiento.DataSet.Next;
    end;
