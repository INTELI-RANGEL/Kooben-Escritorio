    ...
    try
      ...
      begin
        if Not CargarDatosFiltrados(cdEntradaGeneralUpt, 'IdRegistroMovimientoGeneral', [cdBuscarEntradaGeneral.FieldByName('IdRegistroMovimientoGeneral').AsInteger]) then
          raise InteligentException.CreateByCode(6, ['Entradas al Almac�n General', cdBuscarEntradaGeneral.FieldByName('IdEntradaGeneral').AsInteger, 'Id. Entrada General']);
        ...
        if Not CargarDatosFiltrados(cdEntradaGeneralDatosUpt, 'IdRegistroMovimientoGeneral', [cdBuscarEntradaGeneral.FieldByName('IdRegistroMovimientoGeneral').AsInteger]) then
          raise InteligentException.CreateByCode(6, ['Partidas de Entradas al Almac�n General', cdBuscarEntradaGeneral.FieldByName('IdEntradaGeneral').AsInteger, 'Id. Entrada General']);
  ...
  try
    ...
    try
      ...
      if Not CargarDatosFiltrados(cdEntradaGeneralUpt, 'IdRegistroMovimientoGeneral', [-9]) then
        raise InteligentException.CreateByCode(16, ['*Entrada Almac�n General']);
...
var
  CadPaso: String;
begin
  // Generar la cadena de Marcas correspondientes a este insumo (Solo en caso de edici�n)
  Try
    cdMarca.First;
    CadPaso := '';
      ...
      begin
        if cdBuscarEntradaGeneral.ProviderName = '' then
          if Not CrearConjunto(cdBuscarEntradaGeneral, 'cmt_registromovimientogeneral', ccSelect) then
            raise InteligentException.CreateByCode(5, ['Entradas Almac�n General']);
  ...
  try
    ...
    try
      ...
      if Not CrearConjunto(cdAutorizo, 'usuarios', ccCatalog) then
        raise InteligentException.CreateByCode(5, ['Usuarios (Autoriz�)']);
      ...
      if cdRecibio.RecordCount = 0 then
        raise InteligentException.CreateByCode(22, ['Usuarios (Recibi�)']);
  ...
  try
    ...
    try
      ...
      begin
        ...
        raise InteligentException.Create('Debes seleccionar un insumo v�lido para agregar a la partida.');
      end;
      ...
      begin
        ...
        raise InteligentException.Create('Selecciona una presentaci�n para el insumo.');
      end;
      ...
      try
        // Tomar los datos como registro de Edici�n;
        cdEntradaGeneralDatosUpt.close;         //esto lo puse, aunque no queria por que me est� dando pedos con el dataset
