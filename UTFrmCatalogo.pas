...
uses
  ...
  NxColumns, NxDBColumns, dxSkinsForm, cxStyles, dxSkinscxPCPainter,
  cxCustomData, cxFilter, cxData, cxDataStorage, cxNavigator, cxDBData,
  cxGridLevel, cxGridCustomView, cxGridCustomTableView, cxGridTableView,
  cxGridDBTableView, cxGrid, dxRibbon;
...
type
  ...
  TFrmCatalogo = class(TForm)
    ...
    CxDbGridDatos: TcxGridDBTableView;
    ...
    procedure CxDbGridDatosCellDblClick(Sender: TcxCustomGridTableView;
      ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
      AShift: TShiftState; var AHandled: Boolean);
    procedure CxDbGridDatosSelectionChanged(Sender: TcxCustomGridTableView);
  private
...
function TFrmCatalogo.abrir: Boolean;
var
...
begin
  ...
  Try
    if (VerificaPermisos(EntityName, [vpLeer], EADError)) and (Not ModoSeleccion) then
      Try
        ...
        if not CrearConjunto(cdDatos, EntityName, ProviderName, ccCatalog) then
          raise InteligentException.CreateByCode(2, ['No se pudo crear el acceso de datos para la tabla ' + EntityName]);
        ...
        if UsePaginacion then
          ...
          if Not CargarDatosFiltrados(cdDatos, keyfield, [-9]) then
            raise InteligentException.CreateByCode(2, ['No se pudo acceder a la estructura de la tabla ' + EntityName]);
...
begin
  Try
    ...
    try
      VerificaPermisos(EntityName, [vpInsertar], EADError);

      // Acceder en modo update a los datos
      if not CrearConjunto(cdUpt, entityName, providerNameUpt, ccUpdate) then
        raise InteligentException.CreateByCode(5, [Self.ModuloPantalla]);

      Try
        if not CargarDatosFiltrados(cdUpt, keyField, [-9]) then
          raise InteligentException.CreateByCode(6, [Self.ModuloPantalla,cdDatos.FieldByName(keyField).AsString,keyField]);
...
procedure TFrmCatalogo.CxDbGridDatosCellDblClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin

  if CxDbGridDatos.ViewData.RowCount < 1 then
    Exit;
...
procedure TFrmCatalogo.CxDbGridDatosSelectionChanged(
...
begin
  ...
  CxDbGridDatos.FilterRow.Visible := Not CxDbGridDatos.FilterRow.Visible;
end;
...
{  if not ClientModule1.LeePermisos(ModuloPantalla,'IMPORTARXLS') then
  ...
  Try
    Try
      VerificaPermisos(EntityName, [vpImportar], EADError);

      LocEstructura := TClientDataSet.Create(Nil);
      LocForaneas := TClientDataSet.Create(Nil);
      LocDestino := TClientDataSet.Create(Nil);

      CrearConjunto(LocEstructura, EntityName, ccSelect);
      CrearConjunto(LocForaneas, EntityName, ccSelect);
      CrearConjunto(LocDestino, EntityName, LocDes, ccSelect);

      PonerSentenciaSQL(LocEstructura.ProviderName, 'show full columns from ' + EntityName);
      LocEstructura.Open;

      PonerSentenciaSQL(LocForaneas.ProviderName, 'select ' +
                                'TABLE_SCHEMA, ' +
                                'TABLE_NAME, ' +
                                'COLUMN_NAME, ' +
    ...
    End;
  Finally
    EliminarConjunto([LocEstructura, LocForaneas, LocDestino]);
      ...
      Begin
        ...
        begin
          ...
          begin
            ...
            if mdEstructura.FieldByName('iAncho').AsFloat > 255 then
              Excel.ActiveSheet.Columns[mdEstructura.RecNo].ColumnWidth := 255
            else
...
begin
  Repeat
    ...
    Try
      ...
      VerificaPermisos(EntityName, [vpEscribir], EADError);
          ...
          try
            ...
            Try
              ...
              if cdUpt.RecordCount <> 1 then
                raise InteligentException.CreateByCode(3, [cdDatos.FieldByName(codeField).AsString, cdTexto + entityName]);
...
begin
  if CdDatos.State = dsBrowse then
  Try
    VerificaPermisos(EntityName, [vpBorrar], EADError);
    ...
    if ((cxDbGridDatos.Controller.SelectedRowCount = 1) and (InteliDialog.ShowModal('Eliminación de registros', 'Se procederá a borrar el registro [' + cdDatos.FieldByName(CodeField).AsString + ']' + #10 + #10 + '¿Desea proceder con esto ahora?', mtWarning, [mbYes, mbNo], 0) = mrNo)) or
       ((cxDbGridDatos.Controller.SelectedRowCount > 1) and (InteliDialog.ShowModal('Eliminación de registros', 'Se procederá con el borrado de ' + IntToStr(cxDbGridDatos.Controller.SelectedRowCount) + ' registros' + #10 + #10 + '¿Desea borrar estos registros ahora?', mtConfirmation, [mbYes, mbNo], 0) = mrNo)) then
      raise InteligentException.Create('*');

    CtaSel := cxDbGridDatos.Controller.SelectedRowCount;
    if UsePaginacion then
      IdActual := -1;
...
begin
  ...
  Try
    Try
      VerificaPermisos(EntityName, [vpExportar], EADError);

      if DBGridPrincipal.RowCount < 1 Then
        Raise Exception.Create('No hay registros.');

      if not ClientModule1.LeePermisos(ModuloPantalla,'EXPORTARXLS') then
        Raise Exception.Create('Usted no tiene los permisos necesarios para generar la exportación de datos');
      ...
      cdDatos.DisableControls;
      try
        ExportarExcel(cdDatos, ClientModule1.cdOrganizacion,DBGridPrincipal,Self.Caption,
        ...
        'Ciudad del Carmen, Campeche', pbProgress, pnlProgress);
      finally
        if cdDatos.BookmarkValid(Marca) then
          cdDatos.GotoBookmark(Marca);
        cdDatos.EnableControls;
      end;
    Except
      on e:exception do
