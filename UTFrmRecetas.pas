...
uses
  ...
  Dialogs, ClientModuleUnit1, UInteliDialog, URegistro, ComCtrls, cxGraphics,
  ...
  cxButtons, JvExComCtrls, JvListView, ExtCtrls, cxContainer, cxEdit,
  cxTreeView, StdCtrls, DB, UControlExcepciones;
...
begin
  ...
  FrmRecetas.Btn_Abrir.Enabled := pListView.Items.Count > 0;
end;

procedure TFrmRecetas.FormClose(Sender: TObject;
...
procedure TFrmRecetas.FormShow(Sender: TObject);
var
  Cursor: TCursor;
...
procedure TFrmRecetas.lvPlazasDblClick(Sender: TObject);
begin
  if Assigned(lvPlazas.Selected) then
    Btn_Abrir.Click;
end;

procedure TFrmRecetas.rgTipoPlazasClick(Sender: TObject);
begin
  CambiarDatos(dsCatalogoPlazas.DataSet, tvEstructura.Selected, lvPlazas, rgTipoPlazas);
end;

procedure TFrmRecetas.Btn_AbrirClick(Sender: TObject);
var
  Codigo: string;
begin
  if Assigned(lvPlazas.Selected) then
...
procedure TFrmRecetas.Button1Click(Sender: TObject);
begin
  lvPlazas.GroupView := True;
end;

procedure TFrmRecetas.EstiloVistaClick(Sender: TObject);
begin
...
procedure TFrmRecetas.tvEstructuraChange(Sender: TObject;
  Node: TTreeNode);
begin
{procedure CambiarDatos(pDataSet: TDataSet; Node: TTreeNode;
