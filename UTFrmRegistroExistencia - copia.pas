...
begin
  CancelarTransaccion;
  EliminarConjunto([cdExistencia, cdExistenciaPartidaUpt, cdExistenciaPartida, cdEmbarcacion, cdElaboro, cdAutorizo, cdBuscarExistencia, cdRegistroExistencia, cdInsumo]);
  Action := caFree;
end;
...
begin
  ...
  try
    ...
    try
      ...
      if Not CargarDatosFiltrados(cdExistencia, 'IdExistencia', [-9]) then
      ...
      begin
        ...
        if Not CargarDatosFiltrados(cdExistenciaPartidaUpt, 'IdExistencia', [cdExistencia.FieldByName('IdExistencia').AsInteger]) then
          raise InteligentException.CreateByCode(6, ['Partidas del Registro de Existencias (Inventario)', cdExistencia.FieldByName('IdExistencia').AsInteger, 'Id. Existencia']);
  ...
  try
    ...
    try
      ...
      begin
        ...
        if Not CargarDatosFiltrados(cdExistencia, 'IdExistencia', [LastId]) then
      ...
      begin
        case CualGenerar of
          gVacio: if Not CargarDatosFiltrados(cdRegistroExistencia, 'IdEmbarcacion,IdOmitir,Modo', [cdEmbarcacion.FieldByName('IdEmbarcacion').AsInteger, LastId, 'Vacio']) then
                    raise InteligentException.CreateByCode(6, ['Registro de Existencias', cdEmbarcacion.FieldByName('IdEmbarcacion').AsString + ' - ' + IntToStr(LastId) + ' - Vacio', 'Id. Embarcacion - Omitir - Modo']);
          gCeroExistencia: if Not CargarDatosFiltrados(cdRegistroExistencia, 'IdEmbarcacion,IdOmitir,Modo', [cdEmbarcacion.FieldByName('IdEmbarcacion').AsInteger, LastId, 'CeroConExistencia']) then
                             raise InteligentException.CreateByCode(6, ['Registro de Existencias', cdEmbarcacion.FieldByName('IdEmbarcacion').AsString + ' - ' + IntToStr(LastId) + ' - CeroConExistencia', 'Id. Embarcacion - Omitir - Modo']);
          gConExistencia: if Not CargarDatosFiltrados(cdRegistroExistencia, 'IdEmbarcacion,IdOmitir,Modo', [cdEmbarcacion.FieldByName('IdEmbarcacion').AsInteger, LastId, 'ValorConExistencia']) then
                            raise InteligentException.CreateByCode(6, ['Registro de Existencias', cdEmbarcacion.FieldByName('IdEmbarcacion').AsString + ' - ' + IntToStr(LastId) + ' - ValorConExistencia', 'Id. Embarcacion - Omitir - Modo']);
          gCeroTodos: if Not CargarDatosFiltrados(cdRegistroExistencia, 'IdEmbarcacion,IdOmitir,Modo', [cdEmbarcacion.FieldByName('IdEmbarcacion').AsInteger, LastId, 'CeroTodos']) then
                        raise InteligentException.CreateByCode(6, ['Registro de Existencias', cdEmbarcacion.FieldByName('IdEmbarcacion').AsString + ' - ' + IntToStr(LastId) + ' - CeroTodos', 'Id. Embarcacion - Omitir - Modo']);
          gTodos: if Not CargarDatosFiltrados(cdRegistroExistencia, 'IdEmbarcacion,IdOmitir,Modo', [cdEmbarcacion.FieldByName('IdEmbarcacion').AsInteger, LastId, 'Todos']) then
                    raise InteligentException.CreateByCode(6, ['Registro de Existencias', cdEmbarcacion.FieldByName('IdEmbarcacion').AsString + ' - ' + IntToStr(LastId) + ' - Todos', 'Id. Embarcacion - Omitir - Modo']);
        end;
        ...
        try
          ...
          begin
            ...
            cdExistenciaPartidaUpt.FieldByName('IdExistenciaPartida').AsInteger := 0;
            cdExistenciaPartidaUpt.FieldByName('IdExistencia').AsInteger := cdExistencia.FieldByName('IdExistencia').AsInteger;
          ...
          if Not CargarDatosFiltrados(cdExistenciaPartida, 'IdExistencia', [cdExistencia.FieldByName('IdExistencia').AsInteger]) then
            raise InteligentException.CreateByCode(6, ['Partidas del Registro de Existencias (Inventario)', cdExistencia.FieldByName('IdExistencia').AsInteger, 'Id. Existencia']);
        ...
        mrYes: begin
          ...
          begin
            ...
            repeat
              ...
              Lista[Cta] := cdExistenciaPartida.FieldByName('IdExistenciaPartida').AsInteger;
              Inc(Cta);
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          YaAbierto := (LocDataSet.ProviderName <> '') and (LocDataSet.Active) and (LocDataSet.RecordCount > 0) and (LocDataSet.FieldByName('IdExistencia').AsInteger = cdBuscarExistencia.FieldByName('IdExistencia').AsInteger);
  ...
  begin
    if Not CargarDatosFiltrados(cdExistencia, 'IdExistencia', [cdBuscarExistencia.FieldByName('IdExistencia').AsInteger]) then
      raise InteligentException.CreateByCode(6, ['Registros de Existencia (Inventarios)', cdBuscarExistencia.FieldByName('IdExistencia').AsInteger, 'Id. Registro de Exitencia']);

    if Not CargarDatosFiltrados(cdExistenciaPartida, 'IdExistencia', [cdBuscarExistencia.FieldByName('IdExistencia').AsInteger]) then
      raise InteligentException.CreateByCode(6, ['Partidas de Registro de Existencia (Inventarios)', cdBuscarExistencia.FieldByName('IdExistencia').AsInteger, 'Id. Registro de Exitencia']);
  ...
  begin
    ...
    Try
      ...
      try
        ...
        if not CargarDatosFiltrados(cdExistenciaPartidaUpt, 'IdExistencia,IdInsumo', [cdExistencia.FieldByName('IdExistencia').AsInteger, cdExistenciaPartida.FieldByName('IdInsumo').AsInteger]) then
          raise InteligentException.CreateByCode(6, ['Partidas de registro de Existencia', cdExistencia.FieldByName('IdExistencia').AsString + ' - ' + cdExistenciaPartida.FieldByName('IdInsumo').AsString, 'Id. Existencia - Id. Insumo']);
...
begin
  ...
  if Not CargarDatosFiltrados(cdExistenciaPartidaUpt, 'IdExistenciaPartida', [-9]) then
    raise InteligentException.CreateByCode(16, ['Registro de Partidas de Existencias (Inventarios)']);

  cdExistencia.Open;
  cdExistencia.Append;
  cdExistencia.FieldByName('IdExistencia').AsInteger := 0;
